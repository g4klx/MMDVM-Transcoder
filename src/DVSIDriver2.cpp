...
void CDVSIDriver2::reset()
{
  DEBUG1("Resetting the AMBE3000 2");
...
bool CDVSIDriver2::ready() const
{
  return digitalRead(AMBE3000_RTS) == LOW;
}

void CDVSIDriver2::write(const uint8_t* buffer, uint16_t length)
{
  m_serial.write(buffer, length);
}

uint16_t CDVSIDriver2::read(uint8_t* buffer)
{
  while (m_serial.available() > 0) {
