...
unsigned int CFEC::regenerateDStar(uint8_t* bytes) const
{
	uint32_t a = 0U;
	uint32_t b = 0U;
	uint32_t c = 0U;
	...
	for (unsigned int i = 0U; i < 24U; i++) {
		if (READ_BIT1(bytes, DSTAR_A_TABLE[i]))
			a |= MASK;
		if (READ_BIT1(bytes, DSTAR_B_TABLE[i]))
			b |= MASK;
		if (READ_BIT1(bytes, DSTAR_C_TABLE[i]))
			c |= MASK;
		MASK >>= 1;
	}
	...
	MASK = 0x800000U;
	for (unsigned int i = 0U; i < 24U; i++) {
		WRITE_BIT1(bytes, DSTAR_A_TABLE[i], a & MASK);
		WRITE_BIT1(bytes, DSTAR_B_TABLE[i], b & MASK);
		WRITE_BIT1(bytes, DSTAR_C_TABLE[i], c & MASK);
		MASK >>= 1;
	}
...
unsigned int CFEC::regenerateDMR(uint8_t* bytes) const
{
	uint32_t a = 0U;
	uint32_t MASK = 0x800000U;
	for (unsigned int i = 0U; i < 24U; i++, MASK >>= 1) {
		unsigned int aPos = DMR_A_TABLE[i];
		if (READ_BIT1(bytes, aPos))
			a |= MASK;
	}
	...
	for (unsigned int i = 0U; i < 24U; i++, MASK >>= 1) {
		unsigned int aPos = DMR_A_TABLE[i];
		WRITE_BIT1(bytes, aPos, a & MASK);
	}

	MASK = 0x400000U;
	for (unsigned int i = 0U; i < 23U; i++, MASK >>= 1) {
		unsigned int bPos = DMR_B_TABLE[i];
		WRITE_BIT1(bytes, bPos, b & MASK);
	}

	MASK = 0x1000000U;
	for (unsigned int i = 0U; i < 25U; i++, MASK >>= 1) {
		unsigned int cPos = DMR_C_TABLE[i];
		WRITE_BIT1(bytes, cPos, c & MASK);
	}

	return errors;
}

unsigned int CFEC::regenerateIMBE(uint8_t* bytes) const
{
	bool orig[144U];
	bool temp[144U];
	...
	for (unsigned int i = 0U; i < 144U; i++) {
		...
		orig[i] = temp[i] = READ_BIT1(bytes, n);
	}
	...
	uint32_t c0data = CGolay24128::decode23127(g1);
	uint32_t g2 = CGolay24128::encode23127(c0data);
	for (int i = 23; i >= 0; i--) {
	...
	for (unsigned int i = 0U; i < 23U; i++)
		g1 = (g1 << 1) | (bit[i] ? 0x01U : 0x00U);
	uint32_t c1data = CGolay24128::decode23127(g1);
	...
	for (unsigned int i = 0U; i < 23U; i++)
		g1 = (g1 << 1) | (bit[i] ? 0x01U : 0x00U);
	uint32_t c2data = CGolay24128::decode23127(g1);
	...
	for (unsigned int i = 0U; i < 23U; i++)
		g1 = (g1 << 1) | (bit[i] ? 0x01U : 0x00U);
	uint32_t c3data = CGolay24128::decode23127(g1);
	...
	for (unsigned int i = 0U; i < 144U; i++) {
		unsigned int n = IMBE_INTERLEAVE[i];
		WRITE_BIT1(bytes, n, temp[i]);
	}

	return errors;
}

unsigned int CFEC::regenerateDStar(uint32_t& a, uint32_t& b) const
{
	uint32_t orig_a = a;
	uint32_t orig_b = b;
...
unsigned int CFEC::regenerateDMR(uint32_t& a, uint32_t& b, uint32_t& c) const
{
	uint32_t orig_a = a;
	uint32_t orig_b = b;
	...
	uint32_t datb = CGolay24128::decode23127(b);
