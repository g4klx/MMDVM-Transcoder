...
const uint16_t PACKET_LENGTH = 1500U;
...
bool CUDPSocket::setDestination(const std::string& address, unsigned short port)
{
	return lookup(address, port, m_address, m_addressLength) == 0;
}
...
bool CUDPSocket::open()
{
	...
	hints.ai_family = m_address.ss_family;
	...
	m_fd = ::socket(m_address.ss_family, SOCK_DGRAM, 0);
	if (m_fd < 0) {
#if defined(_WIN32) || defined(_WIN64)
...
int16_t CUDPSocket::read(uint8_t* buffer, uint16_t length)
{
...
int16_t CUDPSocket::write(const uint8_t* buffer, uint16_t length)
{
	assert(buffer != nullptr);
	assert(length > 0U);
#if defined(_WIN32) || defined(_WIN64)
...
#if defined(_WIN32) || defined(_WIN64)
	int ret = ::sendto(m_fd, (char*)buffer, length, 0, (sockaddr*)&m_address, m_addressLength);
#else
	ssize_t ret = ::sendto(m_fd, (char*)buffer, length, 0, (sockaddr*)&m_address, m_addressLength);
#endif
